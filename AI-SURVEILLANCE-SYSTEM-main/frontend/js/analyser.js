// API Configuration
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/live-anomaly`, { method: 'POST' });
        const data = await response.json();
    ...
    const anomaliesText = displayText.split('CUSTOM ANOMALIES DETECTED:')[1].trim();

    // Tell backend so Expo app can pick it up
    await fetch(`${API_BASE}/set-latest-anomaly`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            has_anomaly: true,
            report: anomaliesText,
            ...
            document.getElementById('live-reports').textContent = displayText;
        } else {
            showError(data.error);
        }
    } catch (error) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/upload-video-anomalies`, { method: 'POST', body: formData });
        const data = await response.json();
    ...
    await fetch(`${API_BASE}/set-latest-anomaly`, {
        ...
        body: JSON.stringify({
            ...
            document.getElementById('video-results').textContent = displayText;
        } else {
            showError(data.error);
        }
    } catch (error) {
