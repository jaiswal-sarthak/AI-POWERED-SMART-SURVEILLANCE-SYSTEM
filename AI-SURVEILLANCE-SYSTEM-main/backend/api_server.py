        ...
        try:
            ...
            if current_time - last_anomaly_check >= 5 and len(frame_accumulator) >= 3:
                ...
                if len(recent_frames) >= 3:
                    ...
                    # Update latest anomaly for mobile app
                    # Update latest anomaly for mobile app
                    ...
                    print(f"[LATEST_ANOMALY] updated at {latest_anomaly['timestamp']} - report length {len(latest_anomaly['report'])}")
...
async def upload_video_anomalies(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Update latest anomaly for mobile app
        global latest_anomaly
        if "No significant anomalies detected" not in anomaly_report:
            latest_anomaly = {
        "has_anomaly": True,
...
async def process_live_anomaly():
    ...
    try:
        ...
        # Update latest anomaly for mobile app
        global latest_anomaly
        if "No significant anomalies detected" not in anomaly_result:
            latest_anomaly = {
        "has_anomaly": True,
        "report": anomaly_result,
        "timestamp": datetime.utcnow().isoformat()
...
async def get_latest_anomaly():
    return JSONResponse({**latest_anomaly, "success": True})


@app.post("/api/anomalies/clear")
async def clear_latest_anomaly():
    """Clear the latest anomaly (optional reset)"""
    global latest_anomaly
    latest_anomaly = {
        "has_anomaly": False,
        "report": "",
...
@app.post("/api/set-latest-anomaly")
async def set_latest_anomaly(request: Request):
    global latest_anomaly
    try:
        ...
        return {"success": True, "latest_anomaly": latest_anomaly}
    except Exception as e:
